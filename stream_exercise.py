...
class StreamProcessor(object):
   ...
   def process(self):
      ...
      while count < 10 and total < 200:
         digits = self._stream.read(2)  # Read two digits at a time from the 
                                        # beginning of the stream
         if len(digits) < 2:
            break

         number = int(digits)  # Convert the two digits into a number
         total += number  # Adds that number to a running total
         count += 1
